...
ggsave(file = "../../katdetectr_manusscript/figures/plotSyntheticDataPerformancePerVariantTMB.jpeg", plotPerformancePerVariantTMB, width = 3.5)
```
...
```{r}
statsRuntimes <- resultsPerSample |> 
    group_by(package) |> 
    summarise(
        mean = mean(runTime, na.rm = TRUE) / 60,
        sd = sd(runTime, na.rm = TRUE) / 60
    )

statsRuntimes |> 
    ggplot(aes(
        x = factor(package, levels = arrange(performancePerSample, meanRuntime)$package), 
        y = mean, 
        fill = package)) +
