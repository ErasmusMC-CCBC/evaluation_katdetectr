...
generateSyntheticData <- function(path = "data/") {

    futile.logger::flog.info("Generating synthetic data")
    syntheticData <- setParameters() |>
        generateData()

    futile.logger::flog.info("Annotating the kataegis foci")
    kataegisInformation <- getKataegisInformation(syntheticData)

    futile.logger::flog.info("Converting to MAF format")
    syntheticDataMAF <- convertoMafSynth(syntheticData)

    futile.logger::flog.info("Saving to %s/synthetic_data.RData", path)
    saveData(syntheticData, syntheticDataMAF, kataegisInformation, path)
}
...
setParameters <- function() {
    ...
    parameters <- dplyr::bind_rows(
        tibble::tibble(
            nBackgroundMuts = base::rep(nMutations, 64),
            nKataegisFoci = 0,
            nKataegisMuts = 0,
            expectedIMD = 0,
        ...
        dplyr::mutate(
            parmNames = sprintf(
                "%s_%s_%s_%s%s",
                .data$nBackgroundMuts, .data$nKataegisFoci, .data$nKataegisMuts, .data$expectedIMD, base::ifelse(base::is.na(.data$iteration), "", paste0("_(", .data$iteration, ")"))
            )
        )
...
generateData <- function(parameters) {
    ...
    allSyntheticDataList <- pbapply::pblapply(base::seq_len(base::nrow(parameters)), function(i) {
    ...
    # Add sample name.
    sampleNames <- BiocGenerics::sapply(allSyntheticDataList, function(x) {
        unique(x@sampleNames)
    })
...
saveData <- function(syntheticData, syntheticDataMAF, kataegisInformation, path) {
    dataSynthetic <- list(
    ...
    save(dataSynthetic, file = file.path(path, "synthetic_data.RData"))
}
